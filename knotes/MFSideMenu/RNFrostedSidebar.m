...
@property (nonatomic, strong) UIViewController *contentainerView;
...
- (instancetype)initWithsideViewController:(UIViewController *)ViewController
{
    if (self = [super init]) {
        _isSingleSelect = NO;
        _contentainerView=ViewController;
        //_contentView.targetresign=self;
        MFSideMenuShadow *  shadow = [MFSideMenuShadow shadowWithView:self.view andColor:[UIColor colorWithWhite:0.006 alpha:1.000]];
        [shadow draw];
        ...
        _tintColor = [UIColor colorWithWhite:0.000 alpha:0.730];
        _borderWidth = 2;
        _itemBackgroundColor = [UIColor colorWithWhite:0.000 alpha:0.250];
...
- (void)loadView {
    ...
    [self.view addSubview:self.contentainerView.view];
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    self.contentainerView.view.frame = contentFrame;
    ...
    [self.view insertSubview:self.blurView belowSubview:self.contentainerView.view];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(BOOL finished))completion {
    ...
    if (animated) {
        CGFloat parentWidth = self.view.bounds.size.width;
        CGRect contentFrame = self.contentainerView.view.frame;
...
{
    if ([touch.view isDescendantOfView:_contentainerView.view]) {
...
- (void)didTapItemAtIndex:(NSUInteger)index {
    ...
    if (self.borderColors) {
        ...
        CGPoint shapePosition = [self.view convertPoint:view.center fromView:self.contentainerView.view];
...
- (void)layoutSubviews {
    ...
    self.contentainerView.view.frame = CGRectMake(x, 0, _width, self.parentViewController.view.bounds.size.height);
    self.blurView.frame = self.contentainerView.view.frame;
...
- (void)layoutItems {
   ...
   // self.contentView.view.contentSize = CGSizeMake(0, items * (self.itemSize.height + topPadding) + topPadding);
}
