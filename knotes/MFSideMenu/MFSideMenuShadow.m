...
+ (MFSideMenuShadow *)shadowWithView:(UIView *)shadowedView andColor:(UIColor *)color {
    MFSideMenuShadow *shadow = [MFSideMenuShadow shadowWithColor:color radius:0.0f opacity:0.65f];
...
- (id)init {
    self = [super init];
    if(self) {
        self.color = [UIColor colorWithWhite:0.006 alpha:1.000];
        self.opacity = 0.75f;
        self.radius = 0.0f;
        self.enabled = YES;
    }
...
- (void)show {
    ...
    self.shadowedView.layer.shadowOffset = CGSizeMake(0, 0);
