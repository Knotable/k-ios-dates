...
static BOOL jbb_strStartsWith(const char *aString, const char *aPrefix) {
    return strncmp(aPrefix, aString, strlen(aPrefix)) == 0;
}

static BOOL jbb_strCaseStartsWith(const char *aString, const char *aPrefix) {
    return strncasecmp(aPrefix, aString, strlen(aPrefix)) == 0;
}

static const char* jbb_removeObjCTypeQualifiers(const char *aType) {
...
static BOOL jbb_ObjCTypeStartsWith(const char *objCType, const char *targetChar) {
    const char *newObjCType = jbb_removeObjCTypeQualifiers(objCType);

    return strncmp(newObjCType, targetChar, 1);
}

static BOOL jbb_areObjCTypesEqual(const char *lhs, const char *rhs) {
