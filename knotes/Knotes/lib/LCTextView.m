//
//  LCTextView.m
//  RevealControllerProject
//
//  Created by Chen on 9/27/13.
//
//

#import "LCTextView.h"
#define IS_ARC              (__has_feature(objc_arc))

@interface LCTextView (ProtectedMethods)
- (void)_updateShouldDrawPlaceholder;
- (void)_textChanged:(NSNotification *)notification;
@end


@implementation LCTextView

#pragma mark -
#pragma mark Accessors

@synthesize placeholder = _placeholder;
@synthesize placeholderColor = _placeholderColor;

- (void)setText:(NSString *)string {
	[super setText:string];
	[self _updateShouldDrawPlaceholder];
}

- (void)setPlaceholder:(NSString *)string {
	if ([string isEqual:_placeholder]) {
		return;
	}
	
#if IS_ARC
    _placeholder = string;
#else
	[_placeholder release];
	_placeholder = [string retain];
#endif
	
	[self _updateShouldDrawPlaceholder];
}

// added by Chen 05/07/2013
-(void)initialize
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_textChanged:) name:UITextViewTextDidChangeNotification object:self];
    
    self.placeholderColor = [UIColor colorWithWhite:0.702f alpha:1.0f];
    _shouldDrawPlaceholder = NO;
}

#pragma mark -
#pragma mark NSObject

- (void)dealloc {
	[[NSNotificationCenter defaultCenter] removeObserver:self name:UITextViewTextDidChangeNotification object:self];
	
#if !IS_ARC
	[_placeholder release];
	[_placeholderColor release];
	[super dealloc];
#endif
}


#pragma mark -
#pragma mark UIView

- (id)initWithFrame:(CGRect)frame {
	if ((self = [super initWithFrame:frame])) {
        [self initialize];
	}
	return self;
}


- (void)drawRect:(CGRect)rect {
    if ([self backImage] != nil)
    {
        [self.backImage drawInRect:rect blendMode:kCGBlendModeNormal alpha: 1.0f];
    }

	[super drawRect:rect];
	
	if (_shouldDrawPlaceholder) {
		[_placeholderColor set];
		[_placeholder drawInRect:CGRectMake(8.0f, 8.0f, self.frame.size.width - 16.0f, self.frame.size.height - 16.0f) withAttributes: @{ NSFontAttributeName : self.font }];
	}
}


#pragma mark -
#pragma mark Private Methods

- (void)_updateShouldDrawPlaceholder {
	BOOL prev = _shouldDrawPlaceholder;
	_shouldDrawPlaceholder = self.placeholder && self.placeholderColor && self.text.length == 0;
	
	if (prev != _shouldDrawPlaceholder) {
		[self setNeedsDisplay];
	}
}

- (void)_textChanged:(NSNotification *)notificaiton {
	[self _updateShouldDrawPlaceholder];
}

-(void) setValue:(id)value forKey:(NSString *)key
{
    if ([key isEqualToString:@"placeholder"])
    {
        [self setPlaceholder:value];
    }
}

-(void) awakeFromNib
{
    [self setText:@""];
    self.backImage = [UIImage imageNamed:@"textbox_bg.png"];
}

@end
