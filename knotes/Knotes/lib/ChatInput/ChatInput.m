...
//            self.frame = CGRectMake(0, ScreenHeight() - AdjustHeight - 40, ScreenWidth(), 40);
...
//        _sendBtnActiveColor = [UIColor colorWithRed:0.142954 green:0.60323 blue:0.862548 alpha:1];
        ...
        _sendBtnActiveColor = [DesignManager KnoteSelectedColor];
        ...
        [_sendBtn setTitle:@"Post" forState:UIControlStateNormal];
        _sendBtn.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:16.0];
...
- (void) close {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:HIDE_COMMENTINPUTVIEW_NOTIFICATION object:nil userInfo:nil];
...
- (void) resizeView {
    ...
    if (UIInterfaceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation))
    {
        ...
        {
            inputStartingPoint = ScreenWidth();
        }
    }
    else
    {
    ...
    //change for feature
    if (inputStartingPoint <= 480)
    //if (inputStartingPoint <= 352)
    {
		CGFloat offset = (inputStartingPoint - 64);
        self.frame = CGRectMake(0, offset, self.bounds.size.width, -targetHeight);
    }
    else if (inputStartingPoint >= 568)
    {
        CGFloat offset = (inputStartingPoint - 0);
		//change for feature
        if (!self.hidden)
        {
            offset-=108;
...
//    self.frame = CGRectMake(0, 288, self.bounds.size.width, -targetHeight);
...
- (UILabel *) placeholderLabel {
    if (!_placeholderLabel) {
        ...
        _placeholderLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:14.0];
