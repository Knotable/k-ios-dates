...
- (id)initWithImage:(UIImage*)image highlightedImage:(UIImage*)highlightedImage title:(NSString*)title font:(UIFont*)font
{
    ...
    if(self) {
        ...
        _conView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 27, 27)];
        ...
        [_conView.layer setCornerRadius:27/2];
        _icon = [[UIImageView alloc] initWithImage:_image];
        [_icon setContentMode:UIViewContentModeScaleAspectFit];
        [_icon setFrame:CGRectInset(_conView.frame, 3, 3)];
        [_conView addSubview:_icon];
        [self setClipsToBounds:YES];
        CGRect contentFrame = _conView.frame;
        contentFrame.origin = CGPointZero;
        [_conView setFrame:contentFrame];
        _conView.layer.shadowPath = [UIBezierPath bezierPathWithRoundedRect:_conView.bounds cornerRadius:27/2].CGPath;
        ...
        //if(font)
        [_label setFont:[DesignManager knoteWelcomeBoldFontWithSize:12.0f]];
        CGSize labelSize = [_title sizeWithAttributes:@{NSFontAttributeName : _label.font}];
        [_label setFrame:CGRectMake(0, 0, labelSize.width+20, labelSize.height+3)];
        _label.textAlignment=NSTextAlignmentCenter;
        _label.layer.borderColor = [UIColor blackColor].CGColor;
        _label.layer.borderWidth = 0.3f;
        ...
        CGRect frame = CGRectMake(0, 0,
                                  _labelContainer.frame.size.width + kStackItemInternMargin + _conView.frame.size.width+20,
                                  MAX(_labelContainer.frame.size.height, _conView.frame.size.height)+10);
        [self setFrame:frame];
...
- (void)setLabelPosition:(UPStackMenuItemLabelPosition_e)labelPosition
{
    ...
    switch (_labelPosition) {
        case UPStackMenuItemLabelPosition_left:
            ...
            [_conView setCenter:CGPointMake(self.frame.size.width - (_conView.frame.size.width/2)-5, self.frame.size.height/2)];
        ...
        case UPStackMenuItemLabelPosition_right:
            ...
            [_conView setCenter:CGPointMake((_conView.frame.size.width/2)-5, self.frame.size.height/2)];
...
- (CGPoint)itemCenter
{
    return _conView.center;
}

- (CGPoint)centerForItemCenter:(CGPoint)itemCenter
{
    return CGPointMake(itemCenter.x - (_conView.center.x - self.frame.size.width/2), itemCenter.y);
}
