...
@interface LCNoteTextView()
@end

@implementation LCNoteTextView


+ (void)initialize
{
    if (self == [LCNoteTextView class])
    {
        id appearance = [self appearance];
        [appearance setContentMode:UIViewContentModeRedraw];
        UIColor *hcolor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"notebook-line"]];
        [appearance setHorizontalLineColor:hcolor];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.horizontalLineColor)
    {
        ...
        {
            CGFloat linePointY = (baseOffset + (self.font.lineHeight * line))+4;
    ...
    if (self.verticalLineColor)
    {
        ...
        CGContextMoveToPoint(context, 3.0f, self.contentOffset.y);
        CGContextAddLineToPoint(context, 3.0f, self.contentOffset.y + self.bounds.size.height);
        CGContextClosePath(context);
        CGContextStrokePath(context);
    }
}
